...
if ! command -v jq >/dev/null 2>&1; then
  if [ ! -s "$CSV" ]; then
    echo "timestamp,download_mbps,upload_mbps,ping_ms,jitter_ms,packet_loss,server_name,server_id,isp,gw_ping_ms,gw_loss_pct,cf_ping_ms,cf_loss_pct,g_ping_ms,g_loss_pct,dns_ms,http_ms,status,error" > "$CSV"
  fi
  TS="$(date +"%Y-%m-%d %H:%M:%S")"
...
write_ok_row() {
  ...
  echo "$TS,$DL,$UL,$PING,$JITTER,$LOSS,$SNAME,$SID,$ISP,${gw_avg:-},${gw_loss:-},${cf_avg:-},${cf_loss:-},${g_avg:-},${g_loss:-},${dns_ms:-},${http_ms:-},ok," >> "$CSV"
}
...
write_fail_row() {
  ...
  local ERR
  ERR=$(echo "$MSG" | tr '\n' ' ' | sed 's/,/;/g' | cut -c1-240)
  echo "$TS,,,,,,, ,${gw_avg:-},${gw_loss:-},${cf_avg:-},${cf_loss:-},${g_avg:-},${g_loss:-},${dns_ms:-},${http_ms:-},fail,\"$ERR\"" >> "$CSV"
}
